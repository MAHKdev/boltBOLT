...
export class WorkbenchStore {
  ...
  async importLocalFiles() {
    try {
      ...
      this.setDocuments(files); // Use setDocuments to set imported files
    } catch (error) {
      console.error("Error importing files:", error);
    }
  }

  async #readDirectoryFiles(directoryHandle: FileSystemDirectoryHandle): Promise<FileMap> {
    const files: FileMap = {};
    ...
    for await (const entry of directoryHandle.values()) {
      if (entry.kind === 'file') {
        ...
        files[`/${file.name}`] = { type: 'file', content };
      } else if (entry.kind === 'directory') {
        const subFiles = await this.#readDirectoryFiles(entry);
